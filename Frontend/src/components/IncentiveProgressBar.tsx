...
export function IncentiveProgressBar({ current, target, isEnabled }: IncentiveProgressBarProps) {
  ...
  if (!isEnabled) {
    return (
      <div className="bg-gray-400 text-white px-6 py-2 shadow-sm opacity-50">
  ...
  return (
    <div className="bg-gradient-to-r from-teal-600 to-teal-700 text-white px-6 py-3 shadow-md">
      <div className="max-w-screen-2xl mx-auto flex items-center gap-6">
        ...
        <div className="flex items-center gap-2 min-w-[140px]">
          ...
          <span className="font-bold text-lg">激勵進度</span>
        </div>
        ...
        <div className="flex-1 flex items-center gap-4">
          ...
          <div className="flex items-baseline gap-2 min-w-[140px]">
            <span className={`text-2xl font-bold ${isCompleted ? 'text-yellow-300' : 'text-white'}`}>
              {current}
            </span>
            <span className="text-sm text-white/80">/ {target} 個</span>
          </div>
        </div>
        ...
        {isCompleted && (
          <motion.div
            ...
            className="bg-yellow-300 text-teal-900 px-4 py-2 rounded-full font-bold text-sm whitespace-nowrap"
          >
            🎉 目標達成！
          </motion.div>
        )}
