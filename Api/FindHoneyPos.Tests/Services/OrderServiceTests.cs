...
public class OrderServiceTests : IDisposable
{
    ...
    public async Task CreateAsync_ShouldGenerateOrderNumberFormat()
    {
        ...
        var expectedSeq = BusinessRules.InitialOrderSequence + 1;
        var expectedOrderNumber = $"{BusinessRules.OrderNumberPrefix}{expectedSeq.ToString().PadLeft(BusinessRules.OrderNumberPadding, '0')}";
        result.OrderNumber.Should().Be(expectedOrderNumber);
    }
    ...
    #region GetNextShiftSequenceAsync

    [Fact]
    public async Task GetNextShiftSequenceAsync_NoOrders_ShouldReturnInitialPlusOne()
    {
        var shift = TestDataBuilder.CreateShift();
        _context.Shifts.Add(shift);
        await _context.SaveChangesAsync();

        var result = await _service.GetNextShiftSequenceAsync(shift.Id);

        result.Should().Be(BusinessRules.InitialOrderSequence + 1);
    }

    [Fact]
    public async Task GetNextShiftSequenceAsync_ShouldOnlyCountOrdersInSameShift()
    {
        var shift1 = TestDataBuilder.CreateShift();
        var shift2 = TestDataBuilder.CreateShift();
        _context.Shifts.AddRange(shift1, shift2);
        await _context.SaveChangesAsync();

        // shift1 有一筆訂單
        var order = TestDataBuilder.CreateOrder(dailySequence: 5);
        order.ShiftId = shift1.Id;
        ...
        // shift2 應該從頭開始
        var result = await _service.GetNextShiftSequenceAsync(shift2.Id);
